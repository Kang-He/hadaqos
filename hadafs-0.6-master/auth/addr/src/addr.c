...
#define PRIVILEGED_PORT_CEILING 65535
...
auth_result_t 
hf_auth (dict_t *input_params, dict_t *config_params)
{
  char *name = NULL;
  char *searchstr = NULL;
  ...
  switch (((struct sockaddr *) &peer_info->sockaddr)->sa_family) 
	...
	*service = '\0';
	...
	if (peer_port >= PRIVILEGED_PORT_CEILING) {
	  hf_log ("auth/addr", HF_LOG_ERROR,
		  "client is bound to port %d(greater than %d) which is not privileged",
		  peer_port, PRIVILEGED_PORT_CEILING);
	  return AUTH_DONT_CARE;
	}
	break;
	...
	break;
      ...
      default:
	hf_log ("authenticate/addr", HF_LOG_ERROR,
