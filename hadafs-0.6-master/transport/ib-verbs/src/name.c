...
#define CLIENT_PORT_CEILING 12888
                                    ...
                                    socklen_t *sockaddr_len)
{
        ...
        if (remote_port_data == NULL)
        {
                hf_log (this->xl->name, HF_LOG_DEBUG,
                        ...
                        "Defaulting to %d",
                        this->xl->name, HF_DEFAULT_IBVERBS_LISTEN_PORT);

                remote_port = HF_DEFAULT_IBVERBS_LISTEN_PORT;
        }
        else
        {
        ...
        /* TODO: hf_resolve is a blocking call. kick in some
           non blocking dns techniques */
        ret = hf_resolve_ip6 (remote_host, remote_port,
                              sockaddr->sa_family, 
                                    ...
                                    socklen_t *sockaddr_len)
{
        ...
        if (strlen (connect_path) > UNIX_PATH_MAX) {
                hf_log (this->xl->name, HF_LOG_ERROR,
                        "connect-path value length %"HF_PRI_SIZET" > "
...
#define UNIX_PATH_MAX 108
        ...
        if (strlen (listen_path) > UNIX_PATH_MAX) {
                hf_log (this->xl->name, HF_LOG_ERROR,
                        "option listen-path has value length %"HF_PRI_SIZET" > %d",
                                   ...
                                   socklen_t *addr_len)
{
        ...
        if (listen_port == (uint16_t) -1)
                listen_port = HF_DEFAULT_IBVERBS_LISTEN_PORT;
