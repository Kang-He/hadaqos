...
__ib_verbs_ioq_churn_entry (ib_verbs_peer_t *peer, ib_verbs_ioq_t *entry)
{
        ...
        if (quota > 0) {
                ...
                if  (len >= (options->send_size + 2048)) {
                        hf_log ("transport/ib-verbs", HF_LOG_ERROR,
                ...
                if (!ret) {
                        ...
                        hf_log ("transport/ib-verbs", HF_LOG_DEBUG,
...
*/
                                ...
                                if ((ret = peer->trans->xl->notify (peer->trans->xl, HF_EVENT_POLLIN, 
                                                                    peer->trans, NULL)) == -1) {
                                        hf_log ("transport/ib-verbs",
                                                HF_LOG_DEBUG, 
...
ib_verbs_options_init (transport_t *this)
{
        ...
        default:
                if (temp)
                        hf_log ("transport/ib-verbs", HF_LOG_WARNING,
                                "%s: unrecognized MTU value '%s', defaulting "
                                "to '2048'", this->xl->name,
                                data_to_str (temp));
                else
                        hf_log ("transport/ib-verbs", HF_LOG_TRACE,
...
ib_verbs_init (transport_t *this)
{
        ...
        int32_t i;
        uint64_t windowsize = HF_DEFAULT_IBV_WINDOW_SIZE;
        char *wsizestr = NULL;
...

        {
                ...
                if (!options->device_name) {
                        if (*dev_list) {
                                ...
                                hf_log ("transport/ib-verbs", HF_LOG_CRITICAL,
...
unlock:
        ...
        if (!ret && priv->connected) {
                ret = this->xl->notify (this->xl, HF_EVENT_CHILD_UP, this);
        }
...
ib_verbs_handshake_pollerr (transport_t *this)
{
        ...
        this->xl->notify (this->xl, HF_EVENT_POLLERR, this, NULL);
...
ib_verbs_connect (struct transport *this)
{
        ...
        if (dict_get (options, "non-blocking-io")) {
                ...
                if (hf_string2boolean (nb_connect, &non_blocking) == -1) {
                        hf_log (this->xl->name, HF_LOG_ERROR,
                               ...
                               int poll_in, int poll_out, int poll_err)
{
        ...
        priv->sock = accept (main_sock, 
                             ...
                             &this->peerinfo.sockaddr_len);
        if (priv->sock == -1) {
                hf_log ("ib-verbs/server", HF_LOG_ERROR,
                        "accept() failed: %s",
                        strerror (errno));
                free (this->private);
                free (this);
...
ib_verbs_listen (transport_t *this)
{
        ...
        priv->sock = socket (((struct sockaddr *)&sockaddr)->sa_family, 
                             SOCK_STREAM, 0);
        if (priv->sock == -1) {
                hf_log ("ib-verbs/server", HF_LOG_CRITICAL,
...
struct volume_options options[] = {
        ...
        { .key   = {"transport.window-size"},
          ...
          .min   = HF_MIN_IBV_WINDOW_SIZE,
          .max   = HF_MAX_IBV_WINDOW_SIZE,
        },
        { .key = {NULL} }
};
