 ...
 */
static int
dm_round_internal (int rounds, uint32_t *array, uint32_t *h0, uint32_t *h1)
{
	uint32_t sum = 0;
	int      n = 0;
	uint32_t b0  = 0;
...
uint32_t
hf_dm_hashfn_internal (const char *msg, int len)
{
	...
	for (i = 0; i < full_quads; i++) {
		...
		dm_round_internal (DM_PARTROUNDS, &array[0], &h0, &h1);
	}
...
dm_hash_compute_internal (const char *name, uint32_t *hash_p)
{
	...
	hash = hf_dm_hashfn_internal (name, strlen (name));
...
#define MAKE_RSYNC_FRIENDLY_NAME_INTERNAL(rsync_frndly_name, name) do {          \
...
int
object_path_hash_compute (const char *path, uint32_t *hash_p)
{
	char  *rsync_friendly_name = NULL;

	MAKE_RSYNC_FRIENDLY_NAME_INTERNAL (rsync_friendly_name, path);
