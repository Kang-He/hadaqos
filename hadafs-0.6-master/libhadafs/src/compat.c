...
#define HF_FINDER_INFO_XATTR   "com.apple.FinderInfo"
#define HF_RESOURCE_FORK_XATTR "com.apple.ResourceFork"
#define HF_FINDER_INFO_SIZE    32

static const char hf_finder_info_content[HF_FINDER_INFO_SIZE] = {
...
int32_t 
hf_darwin_compat_listxattr (int len, dict_t *dict, int size)
{
	data_t *data = NULL;
	if (len == -1)
		len = 0;

	data = dict_get (dict, HF_FINDER_INFO_XATTR);
	if (!data) {
		dict_set (dict, HF_FINDER_INFO_XATTR, 
			  bin_to_data ((void *)hf_finder_info_content,
				       HF_FINDER_INFO_SIZE));
		len += strlen (HF_FINDER_INFO_XATTR);
	}

	data = dict_get (dict, HF_RESOURCE_FORK_XATTR);
	if (!data) {
		dict_set (dict, HF_RESOURCE_FORK_XATTR, str_to_data (""));
		len += strlen (HF_RESOURCE_FORK_XATTR);
	}
...
int32_t 
hf_darwin_compat_getxattr (const char *key, dict_t *dict)
{
	data_t *data = NULL;

	if (strcmp(key, HF_FINDER_INFO_XATTR) == 0) {
		data = dict_get (dict, HF_FINDER_INFO_XATTR);
		if (!data) {
			dict_set (dict, HF_FINDER_INFO_XATTR, 
				  bin_to_data ((void *)hf_finder_info_content,
					       HF_FINDER_INFO_SIZE));
			return HF_FINDER_INFO_SIZE;
		}
		return 0;
	}
	
	if (strcmp(key, HF_RESOURCE_FORK_XATTR) == 0) {
		data = dict_get (dict, HF_RESOURCE_FORK_XATTR);
		if (!data) {
			/* Always null */
			dict_set (dict, HF_RESOURCE_FORK_XATTR,
				  str_to_data (""));
			return 0;
		}
		return 0;
...
int32_t 
hf_darwin_compat_setxattr (dict_t *dict)
{
	data_t *data = NULL;
