...
__event_getindex (struct event_pool *event_pool, int fd, int idx)
{
	...
	if (event_pool == NULL) {
		hf_log ("event", HF_LOG_ERROR, "invalid argument");
		return -1;
	}
...
event_pool_new_epoll (int count)
{
	...
	if (epfd == -1) {
		hf_log ("epoll", HF_LOG_ERROR, "epoll fd creation failed (%s)",
			strerror (errno));
		free (event_pool->reg);
		free (event_pool);
		return NULL;
...
#ifndef XIAOW_20200528
    while (!event_pool->fin) {
#else
        while (1) {
#endif
		pthread_mutex_lock (&event_pool->mutex);
