		...
		xlator_t *xl)
{
	...
	HF_VALIDATE_OR_GOTO("transport", options, fail);
	HF_VALIDATE_OR_GOTO("transport", xl, fail);
  
	trans = CALLOC (1, sizeof (struct transport));
	HF_VALIDATE_OR_GOTO("transport", trans, fail);
	...
	if (ret < 0) {
		...
		if (ret < 0) {
			ret = dict_set_str (options,
					    ...
					    "inet");
			if (ret < 0) {
				hf_log ("dict", HF_LOG_ERROR,
					"setting address-family failed");
			}
		}

		hf_log ("transport", HF_LOG_WARNING,
	...
	hf_log ("transport", HF_LOG_DEBUG,
		"attempt to load file %s", name);

	handle = dlopen (name, RTLD_NOW|RTLD_GLOBAL);
	if (handle == NULL) {
		hf_log ("transport", HF_LOG_ERROR, "%s", dlerror ());
		hf_log ("transport", HF_LOG_ERROR,
	...
	if (trans->init == NULL) {
		hf_log ("transport", HF_LOG_ERROR,
			"dlsym (hf_transport_init) on %s", dlerror ());
		FREE (trans);
		goto fail;
	}
	...
	if (trans->fini == NULL) {
		hf_log ("transport", HF_LOG_ERROR,
			"dlsym (hf_transport_fini) on %s", dlerror ());
		FREE (trans);
		goto fail;
	}
                  ...
                  struct iobref *iobref)
{
	...
	HF_VALIDATE_OR_GOTO("transport", this, fail);
	HF_VALIDATE_OR_GOTO("transport", this->ops, fail);
...
transport_unref (transport_t *this)
{
	...
	if (refcount == 0) {
		this->xl->notify (this->xl, HF_EVENT_TRANSPORT_CLEANUP, this);
...
transport_peerproc (void *trans_data)
{
        ...
        while (1) {
                ...
                trans->xl->notify (trans->xl, HF_EVENT_POLLIN, trans);
