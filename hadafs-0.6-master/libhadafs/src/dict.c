		    ...
		    dict_t *new)
{
	...
	HF_VALIDATE_OR_GOTO("dict", dict, fail);

	if (new == NULL) {
		local_new = dict_new ();
		HF_VALIDATE_OR_GOTO("dict", local_new, fail);
		new = local_new;
	}
...
_dict_serialized_length (dict_t *this)
{
	...
	if (count < 0) {
		hf_log ("dict", HF_LOG_ERROR, "count (%d) < 0!", count);
		goto out;
	}
	...
	while (count) {
		...
		if (!pair->key) {
			hf_log ("dict", HF_LOG_ERROR, "pair->key is null!");
			goto out;
		}
...
dict_serialized_length (dict_t *this)
{
	...
	if (!this) {
		hf_log ("dict", HF_LOG_ERROR, "this is null!");
		goto out;
	}
	...
	if (!buf) {
        ...
        { 
                ...
                *buf = CALLOC (1, len);
                if (*buf == NULL) {
                        ret = -ENOMEM;
                        hf_log ("dict", HF_LOG_ERROR, "out of memory");
                        goto unlock;
                }
