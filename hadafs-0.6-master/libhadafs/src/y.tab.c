...
#define YYSTYPE char *
#define HF_CMD_BUFFER_LEN (8 * HF_UNIT_KB) 
...
{
       0,    53,    53,    53,    55,    56,    57,    59,    60,    61,
      62,    63,    64,    65,    67,    67,    69,    69,    71,    71,
      73,    74,    75,    77,    77,    78,    78
};
#endif
...
static const char *const yytname[] =
{
  ...
  "SECTION_FOOTER", "SECTION_DATA", "TYPE_LINE", "SUBSECTION_LINE",
...
yyreduce:
  ...
  switch (yyn)
    {
        case 5:
#line 56 "./spec.y" /* yacc.c:1646  */
    {if( -1 == new_section ((yyvsp[0]))) { YYABORT; } }
#line 1260 "y.tab.c" /* yacc.c:1646  */
    break;

  case 6:
#line 57 "./spec.y" /* yacc.c:1646  */
    {if( -1 == section_end ()) { YYABORT; } }
#line 1266 "y.tab.c" /* yacc.c:1646  */
    break;

  case 14:
#line 67 "./spec.y" /* yacc.c:1646  */
...
cut_tree (xlator_t *tree)
{
        ...
        if (!tree) {
                hf_log ("parser", HF_LOG_DEBUG, "Translator tree not found");
                return -1;
        }

        hf_log ("parser", HF_LOG_DEBUG, "Failed to build translator graph");
...
new_section (char *name)
{
        ...
        tree = node;
        hf_log ("parser", HF_LOG_TRACE, "New node for '%s'", name);
...
section_type (char *type)
{
        ...
        int32_t ret = -1;
        if (!type) {
                hf_log ("parser", HF_LOG_DEBUG, "Invalid argument type");
                return -1;
        }
        ...
        hf_log ("parser", HF_LOG_TRACE, "Type:%s:%s", tree->name, type);
...
section_option (char *key, char *value)
{
        ...
        if (!key || !value){
                ...
                hf_log ("parser", HF_LOG_ERROR, "Invalid argument");
                return -1;
        }
	...
	ret = dict_set (tree->options, key, str_to_data (value));
        ...
        hf_log ("parser", HF_LOG_TRACE, "Option:%s:%s:%s",
                tree->name, key, value);
...
section_sub (char *sub)
{
        ...
        if (!sub) {
                ...
                hf_log ("parser", HF_LOG_ERROR, "Invalid subvolumes argument");
                return -1;
        }
        ...
        hf_log ("parser", HF_LOG_TRACE, "child:%s->%s", tree->name, sub);
...
section_end (void)
{
        ...
        hf_log ("parser", HF_LOG_TRACE, "end:%s", tree->name);
...
execute_cmd (char *cmd, char **result, size_t size)
{
	...
	fpp = popen (cmd, "r");
	if (!fpp) {
		hf_log ("parser", HF_LOG_ERROR, "%s: failed to popen", cmd);
		return -1;
	}
...
parse_backtick (FILE *srcfp, FILE *dstfp)
{
	...
	size_t cmd_buf_size = HF_CMD_BUFFER_LEN;
...
extern FILE *yyin;
xlator_t *
file_to_xlator_tree (hadafs_ctx_t *ctx,
                     FILE *fp)
{
        int32_t ret = 0;
