...
#ifndef XIAOW20180925
        if (iobuf_arena->mem_base == MAP_FAILED) {
		hf_log("iobuf", HF_LOG_ERROR, "arana [arena_size=%ld] mmap failed %s",
...
#endif
#else
       	iobuf_arena->mem_base = valloc(arena_size);
	if (iobuf_arena->mem_base == NULL){
		hf_log("iobuf", HF_LOG_ERROR, "arana [arena_size=%ld] valloc failed %s",
			arena_size, strerror(errno));
                goto err;
	}
#endif
...
iobuf_get (struct iobuf_pool *iobuf_pool)
{
        ...
        pthread_mutex_lock (&iobuf_pool->mutex);
        {
                ...
                if (!iobuf_arena){
#ifndef XIAOW20180925
			hf_log("iobuf", HF_LOG_ERROR, "iobuf_arena empty");
#endif
                        goto unlock;
		}
                ...
                if (!iobuf){
#ifndef XIAOW20180925
			hf_log("iobuf", HF_LOG_ERROR, "iobuf empty");
#endif
                        goto unlock;
		}
                __iobuf_ref (iobuf);
