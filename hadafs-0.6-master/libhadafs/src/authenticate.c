      ...
      void *data)
{
	...
	if (!strncasecmp (key, "ip", strlen ("ip"))) {
		hf_log ("authenticate", HF_LOG_ERROR,
	...
	if (!handle) {
		hf_log ("authenticate", HF_LOG_ERROR, "dlopen(%s): %s\n", 
			auth_file, dlerror ());
		dict_set (this, key, data_from_dynptr (NULL, 0));
		FREE (auth_file);
		*error = -1;
	...
	authenticate = dlsym (handle, "hf_auth");
	if (!authenticate) {
		hf_log ("authenticate", HF_LOG_ERROR,
			"dlsym(hf_auth) on %s\n", dlerror ());
		dict_set (this, key, data_from_dynptr (NULL, 0));
	...
	if (!auth_handle) {
		hf_log ("authenticate", HF_LOG_ERROR, "Out of memory");
		dict_set (this, key, data_from_dynptr (NULL, 0));
		*error = -1;
		return;
	}
	...
	if (auth_handle->vol_opt->given_opt == NULL) {
		hf_log ("authenticate", HF_LOG_DEBUG,
...
int32_t
hf_auth_init (xlator_t *xl, dict_t *auth_modules)
{
	int ret = 0;
	...
	if (ret) {
		hf_log (xl->name, HF_LOG_ERROR, "authentication init failed");
...
auth_result_t 
hf_authenticate (dict_t *input_params, 
...
void 
hf_auth_fini (dict_t *auth_modules)
{
	int32_t dummy;
