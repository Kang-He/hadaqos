...
#define HF_MEM_POOL_PAD_BOUNDARY         (sizeof(struct list_head))
#define mem_pool_chunkhead2ptr(head)     ((head) + HF_MEM_POOL_PAD_BOUNDARY)
#define mem_pool_ptr2chunkhead(ptr)      ((ptr) - HF_MEM_POOL_PAD_BOUNDARY)
		 ...
		 unsigned long count)
{
	...
	if (!sizeof_type || !count) {
		hf_log ("mem-pool", HF_LOG_ERROR, "invalid argument");
		return NULL;
	}
        padded_sizeof_type = sizeof_type + HF_MEM_POOL_PAD_BOUNDARY;
