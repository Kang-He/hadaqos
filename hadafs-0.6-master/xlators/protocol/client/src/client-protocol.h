...
#define HF_CLIENT_INODE_SELF   0
#define HF_CLIENT_INODE_PARENT 1
...
struct _client_conf {
	...
	/* enhancement for 'forget', a must required where lot 
	   of stats happening */
	struct {
		char  *path_array[CLIENT_PROTO_FORGET_LIMIT + 4];
		uint32_t pathlen_array[CLIENT_PROTO_FORGET_LIMIT + 4];
		uint32_t  count;
...
struct client_connection {
	...
	hf_timer_t          *reconnect;
	char                 connected;
	uint64_t             max_block_size;
	hf_timer_t          *timer;
	hf_timer_t          *ping_timer;
...
static inline void
hf_string_to_stat(char *string, struct stat *stbuf)
{
	uint64_t dev        = 0;
	uint64_t ino        = 0;
	uint32_t mode       = 0;
	...
	sscanf (string, HF_STAT_PRINT_FMT_STR,
		&dev,
		&ino,
		&mode,
		&nlink,
